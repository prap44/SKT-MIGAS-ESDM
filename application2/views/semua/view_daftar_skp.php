	...
	<?php } ?>
		<div class="columnfull">
			<div class="borderbottomdotted">
				<legend>Pengajuan SK Penunjukkan Baru Masuk</legend>
			</div>
							...
							if($select_last_disposisi_by_id_permohonan != NULL){
								if ($select_last_disposisi_by_id_permohonan->status_progress == $where && $select_last_disposisi_by_id_permohonan->user_tujuan == $this->session->userdata('id_user')) {
										...
										<td style="text-align:right">
											<?= '<a href="'.site_url('all_admin/detail_perusahaan/pengajuan_skp/'.$permohonan->id_permohonan).'">Detail</a> |
											<a href="'.site_url('all_admin/histori_disposisi/pengajuan_skp/'.$permohonan->id_permohonan).'">Histori Progress</a>'; ?>
											<?php if($this->session->userdata('level') != 2){
												if($this->session->userdata('level') != 6){ ?>
												<?= ' | <a href="'.site_url('all_admin/pengajuan_skp_diterima/add/'.$permohonan->id_permohonan).'">Disposisi</a>'; } ?>
		...
		<div class="columnfull">
			<div class="borderbottomdotted">
				<legend>Pengajuan Perpanjangan SK Penunjukkan Masuk</legend>
			</div>
			<div class="tablesection">
				<table class="table table-bordered table-striped">
					<thead>
