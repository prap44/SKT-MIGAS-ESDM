			...
			case 'upload_file':
				...
				else
				{
					...
					else
					{
						...
						$file_url_anchor .= ' target="_blank">Lihat Dokumen';
					}
					$file_url_anchor .= '</a>';
	...
	protected function upload_file($state_info)
	{
		...
		if(isset($this->upload_fields[$state_info->field_name]) )
		{
			if($this->callback_upload === null)
			{
				...
				{
					$callback_before_upload_response = call_user_func($this->callback_before_upload, $_FILES,  $this->upload_fields[$state_info->field_name], $post_data);
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function showList($ajax = false, $state_info = null)
	{
		...
		$data->unset_mytools		= $this->unset_mytools; /* tambahanku */
	...
	protected function get_date_input($field_info,$value)
	{
		...
		$input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='text' value='$date' maxlength='10' class='datepicker-input' readonly/>
		<a class='datepicker-input-clear' tabindex='-1'>".$this->l('form_button_clear')."</a>";
		return $input;
	}
	...
	protected function get_relation_input($field_info,$value)
	{
		...
		$ttl_rz = count($options_array_rz = $this->get_relation_array($field_info->extras));
		if($ttl_rz != 1){
	...
	public function getStateInfo()
	{
		...
		switch ($state_code) {
			...
			case 11:
				if(!empty($_POST) && $first_parameter !== null)
				{
					$state_info = (object)array('field_name' => $first_parameter,'unwrapped_data' => $_POST);
				}
			break;
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	protected $unset_mytools		= null; /* tambahanku */
	...
	/* tambahanku */
	public function unset_mytools()
	{
		$this->unset_mytools = true;
