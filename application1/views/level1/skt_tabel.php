...
if($data_permohonan != NULL){
	if($data_permohonan->selesai == 0){
		if($data_permohonan->jenis_permohonan == 'SKT Baru' || $data_permohonan->jenis_permohonan == 'Perpanjangan SKT'){
			$pesan = 'Anda sedang dalam proses pengajuan permohonan Surat Keterangan Terdaftar(SKT) MIGAS.<br/>Lengkapi data dan selesaikan proses pengajuan yang sedang berjalan untuk bisa mengajukan permohonan lainnya.<br/>Klik tombol <b>Next</b> untuk melanjutkan proses pengajuan.'; 
			$hide = "$('.lanjut').show();";
...
function bidang_usaha123(clicked_id){
	// var bidangusaha = $('#bidang_usaha').find('option:selected').val();
		...
		<ul class="option" style="list-style: none;">
			...
			<div class="list-bidang">
				...
				<select id="bidang_usaha" name="bidang_usaha" onchange="bidang_usaha123();">
					...
					<option id="<?=  $bidang_usaha->id_bidang_usaha; ?>"  value="<?=  $bidang_usaha->id_bidang_usaha; ?>"><?=  $bidang_usaha->bidang_usaha; ?></option>
				<?php endforeach; ?>
			</select>
