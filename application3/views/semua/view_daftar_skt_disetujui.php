	...
	<?php } ?>
		<div class="columnfull">
			<div class="borderbottomdotted">
				<legend>Pengajuan SKT Baru Disetujui</legend>
			</div>
							...
							if($select_last_disposisi_by_id_permohonan != NULL){
								if ($select_last_disposisi_by_id_permohonan->status_progress == $where && $select_last_disposisi_by_id_permohonan->user_tujuan == $this->session->userdata('id_user')) {
										...
										<td style="text-align:right">
											<?= '<a href="'.site_url('all_admin/detail_evaluasi/pengajuan_skt/'.$permohonan->id_permohonan).'">Detail</a> |
											<a href="'.site_url('all_admin/histori_disposisi/pengajuan_skt_admin_naik/'.$permohonan->id_permohonan).'">Histori Progress</a>'; ?>
											<?php if($this->session->userdata('level') != 2):
												if($this->session->userdata('level') != 6){ ?>
												<?= ' | <a href="'.site_url('all_admin/pengajuan_skt_diterima_naik/add/'.$permohonan->id_permohonan).'">Disposisi</a>'; } ?>
		...
		<div class="columnfull">
			<div class="borderbottomdotted">
				<legend>Pegajuan Perpanjangan SKT Disetujui</legend>
			</div>
			<div class="tablesection">
				<table class="table table-bordered table-striped">
					<thead>
