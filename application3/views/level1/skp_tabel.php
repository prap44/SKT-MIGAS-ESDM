								...
								<div id="pilihan_bidang">
									...
									<div class="column-bidang">
										...
										<div class="list-bidang" id="jika_perpanjangan">
											...
											if($nomor_skt != NULL){
												echo '<select name="file_dokumen_skt" id="no_skt"  onchange="pilih_skt(this)"><option value="">-Pilih No SKT-</option>';
												foreach($nomor_skt as $nom){
													echo '<option value="'.$nom->file_dokumen.'" name="'.$nom->id_dokumen.'" id="dokumen-'.$nom->id_dokumen.'">'.$nom->no_dokumen.'</option>'; ?>
												<script>
													function pilih_skt(clicked) {
														$element = document.getElementById('file_skt');
														$element.innerHTML = '<a href="<?php echo base_url('assets/uploads/file_skt') ?>/'+ clicked.value +'">'+ clicked.value +'</a><input name="id_dokumen" type="hidden" value="'+ clicked.value +'">';
